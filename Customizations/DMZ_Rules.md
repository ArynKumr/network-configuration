...
**Purpose:**  
Ensures traffic hitting the public WAN IP on a specific port from a specific source is redirected to the internal host.  
`insert rule` places this at the **top** of the NAT chain.

```
nft insert rule inet nat NAT_PRE \
    ip saddr <public_remote_ip> ip daddr <public_facing_isp_ip> \
    <protocol> dport <public_facing_isp_port> \
    dnat to <client_ip>:<client_port>
```
...
```
nft insert rule inet nat NAT_PRE \
    ip saddr <public_remote_ip> ip daddr <public_facing_isp_ip> <protocol> \
    dport <starting_public_facing_isp_port>-<ending_public_facing_isp_port> \
    dnat to <client_ip>:<starting_client_port>-<ending_client_port>
```
