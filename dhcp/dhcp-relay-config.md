...
This guide describes a deterministic, production-safe method to deploy an **ISC DHCP Relay Agent** with a **ISC-Kea DHCP Server** when clients and server are separated by Layer-3 boundaries (VLANs, routed ports, bridges).

---

## 1. Architectural Logic

DHCP relies on broadcasts, which do not cross L3 boundaries.

When clients are not on the same L2 segment as the Kea server, a **DHCP Relay Agent** is required.

The relay works by:

1. Intercepting DHCP broadcasts on the client-facing interface
2. Encapsulating the request into a unicast packet
3. Inserting its own IP address into the `giaddr` field
4. Forwarding the request to the Kea server
5. Allowing Kea to select a subnet based on `giaddr`

**Kea selects subnets strictly by matching `giaddr` to `subnet4.relay.ip-address`.**
No match means no lease. Kea does not guess.

---

## 2. Installation and Persistent Configuration
...
## 3. Minimal Kea Server Configuration Required

Kea must have a subnet that matches the relay agent’s `giaddr`.
    ...
    "subnet4": [
      {
        ...
        "relay": {
          "ip-address": "192.168.10.1" //This is the giaddr that is expected
        },
        "option-data": [
...
| Subnet selection | Incoming interface | `giaddr`            |
